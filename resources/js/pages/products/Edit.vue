...
const form = useForm<ProductForm>({
    ...
    crop_id: props.product.crop.id,
    notes: props.product.notes,
});

// `category` only narrows the crop search below — it isn't submitted, the
// product only stores crop_id. Pre-filled from the product's current crop.
const category = ref<number | null>(props.product.crop.category.id);
const cropQuery = ref(props.product.crop.name);
